...
func FormatUsageLine(p UsageLineParams) string {
	...
	return fmt.Sprintf(
		"Modèle: %s ➡️ %s\n"+
			"Entrée: lecture %s + écriture %s + première %s\n"+
			"Sortie: %s\n"+
			"Coût: cette requête %.4f ¥, cumulé %.4f ¥\n"+
			"Cache: taux de hit %.2f%%, taux d'écriture %.2f%%, ratio R/W %.2f",
...
func FormatSummaryLine(s Summary) string {
	...
	return fmt.Sprintf("Stats: taux de hit cache %.1f%%, ratio R/W %.2f, coût cumulé %.2f ¥, tokens total %s",
...
func WriteSummary(w io.Writer, s Summary) {
	...
	if rwRatio > 0 {
		fmt.Fprintf(&buf, "  Ratio R/W cache: %.2f\n", rwRatio)
	}
	fmt.Fprintf(&buf, "  Coût cumulé: ¥%.6f\n", s.TotalCost)
	totalTokens := s.InputTokens + s.OutputTokens
	if totalTokens > 0 {
		fmt.Fprintf(&buf, "  Coût moyen global: %s\n", FormatAvgCost(s.TotalCost, totalTokens))
	}
	for model, ms := range s.ByModel {
		...
		if ms.Cost > 0 {
			...
			fmt.Fprintf(&buf, "    %s: ¥%.6f (%d requêtes, %s entrée, %s sortie, %s)\n",
